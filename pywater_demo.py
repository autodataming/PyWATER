...
2016-11-04 modified all API of PDB website ,and use the 4lyw as example to test, it showed success. 
put the python script into the pmg_tk/startup.
...
def displayInputs( selectedStruturePDB, selectedStrutureChain,
    ...
    logger.debug('enter in func  displayInputs')
    print "enter in func displayputs func"
...
def okMobility( pdbFile, mobilityCutoff = 2.0 ):
    ...
    normBfactors = []
    OccupancyAndBfactor = []
    print "!!!!!!!!!!!!!!!!check water moblity in %s"%(pdbFile)
    ...
    logger.debug( 'Number of water molecules: %d' %nWaters )
    count = 0
    for line in reversed(pdbFileLines):
        if line.startswith('HETATM'):
            m = ((float(line[60:66])/avgB)/(float(line[54:60])/avgO))
            #print "water mob value is",m
            if m >= mobilityCutoff:
                count+=1
                pdbFileLines.remove(line)
    logger.info( 'Water oxygen atoms with a higher mobility than %s: %s in pdbfile %s: ' % (mobilityCutoff, count,pdbFile))
    
    #count is the number of move water;
    #nWater is all water number
    if count > (nWaters/2):
        considerPDB = False
    elif count > 0:
        ...
        logger.info('count is %d should be zero'%(count))
        considerPDB = True
...
class Protein():
    ...
    def calculate_water_coordinates(self, tmp_dir = False):
        ...
        for line in open(path):
            line = line.strip()
            if line.startswith('HETATM'):
                # water oxygen atom number       line[22:30]
                #__repr__ function: pdbid_chain
...
def makePDBwithConservedWaters(ProteinsList, temp_dir, outdir, save_sup_files):
    ...
    for protein in ProteinsList:
        logger.info('creating %s'%(protein))
        #copy water to object
    ...
    cmd.delete('cwm_*')
    logger.info('save all pdb and water pdbs,sleep 5 min')
    time.sleep(30)
    ...
    print "hello,world"
    time.sleep(5)
    #logger.info("proteins length %d" %(length))
    print dir(ProteinsList)
    time.sleep(5)
    #logger.info( 'Protein chains list is %s proteins long.' % (len(ProteinsList.proteins) ))
    pprint(ProteinsList.proteins)
    
    if ProteinsList.refinement != 'No refinement':
        print "i am here line no 468"
        time.sleep(5)
        length = len(ProteinsList.proteins)
        print length
        time.sleep(5)
        if ProteinsList.refinement == 'Mobility':
            print "i am here lineno 479"
            ...
            for protein in reversed(ProteinsList.proteins):
                if str(protein) != str(ProteinsList.selectedPDBChain):
                    print "i am here 483 no"
                    time.sleep(10)
                    tempMobvalue=okMobility(os.path.join(temp_dir, 'cwm_%s_Water.pdb' % protein))
                    time.sleep(1)
                    
                    
                    print "protein %s moblity %s" % (protein, str(tempMobvalue))
                    if not tempMobvalue:
                        ProteinsList.proteins.remove(protein)
                    
                    #okMobility have problem
#                    if not okMobility(os.path.join(temp_dir, 'cwm_%s_Water.pdb' % protein)):
#                        print "i am 452 line no:"
#                        ProteinsList.proteins.remove(protein)
        print "tempMobvalue finished"
        time.sleep(10)
        if ProteinsList.refinement == 'Normalized B-factor': 
            ...
            for protein in reversed(ProteinsList.proteins):
                if str(protein) != str(ProteinsList.selectedPDBChain):
                    tempbfactor=okBfactor(os.path.join(temp_dir, 'cwm_%s_Water.pdb' % protein))
                    print "tempbfactor",tempbfactor
        ...
        if water_coordinates:
            ...
            if len(water_coordinates) != 1:
                ...
                if len(water_coordinates) < 50000:
                    ...
                    #INFO:PyWATER:Number of water molecules to cluster: 24701
                    print "number of waters",len(water_coordinates)
                    
                       #water not float
                    #t parameter 2.4
                    #method complete   
                    print "t parameter",ProteinsList.inconsistency_coefficient
                    print "method",ProteinsList.clustering_method
                    
                    
                    
                    for i in water_coordinates:
                        print i
                    time.sleep(20)



                    #fclusterdata may have problems 
                    #!!!!#!!!!#!!!!
                    #!!!!#!!!!
                    #!!!!
                    #water not float
...
#                        
                    ...
                    print "i am here 586 "
                    a=os.path.join( outdir, selectedPDBChain, '%s_clusterPresence.txt' % selectedPDBChain )
                    print "test 588",a
                    ...
                    for a,b in zip(water_ids,FDlist):
                        print "cluster number:",b
                        #4qb3_a_473, 4zc9_a_4204,
                        print "water ",a
                        if fcDic.has_key(b):
                            fcDic[b].append(a)
                        else:
                    ...
                    for i in ProteinsList.proteins:
                        ...
                        l += 1
                        print "convert protein to str",str(i)
                    ...
                    print "i am 688 here cluster finished"
                    time.sleep(10)
                    ...
                    if selectedPDBChain in conservedWaterDic.keys():
                        ...
                        print "i am here to write pdbconservedwaterout 649 no"

                        # add conserved waters to pdb file
                        cmd.delete('cwm_*')
                        cmd.load( os.path.join(temp_dir, 'cwm_%s.pdb' % selectedPDBChain) )
                        logger.info(' cwm_%s protein should be loaded in PYMOL' %(selectedPDBChain))
                        time.sleep(10)
                        
                        
                        
                        print "i am here to load pdb 658"
...
def isXray( pdb ):
    ...
    logging.debug(expInfoAddress)
    print expInfoAddress
...
def chainPresent(pdb,chain):
    ...
    chainInfoAddress='http://www.rcsb.org/pdb/rest/customReport?pdbids=%s&customReportColumns=entityId&service=wsdisplay&format=xml&ssa=n' % pdb
...
#generate pdbchain list
#<pdbChain name="4J0R.A" rank="95"/>
#<pdbChain name="3SVG.A" rank="96"/>
#<pdbChain name="4O7F.A" rank="97"/>
#<pdbChain name="4O7F.B" rank="97"/>
#<pdbChain name="4HBW.A" rank="98"/>
#<pdbChain name="4MEO.A" rank="99"/>
def fetchpdbChainsList( selectedStruture, seq_id ):
    ...
    seqClustAddress = 'http://www.rcsb.org/pdb/rest/sequenceCluster?cluster=%s&structureId=%s' % (seq_id, selectedStruture)
    #http://www.rcsb.org/pdb/rest/sequenceCluster?cluster=95&structureId=4lyw.A
    #http://www.rcsb.org/pdb/rest/sequenceCluster?cluster=40&structureId=4hhb.A
    print seqClustAddress
    #raise NameError    
...
def filterbyResolution( pdbChainsList, resolutionCutoff ):
    ...
    for pdb in pdbsList:
        ...
        getResolutionAddress ='http://www.rcsb.org/pdb/rest/customReport?pdbids=%s&customReportColumns=resolution&service=wsfile&format=xml&ssa=n' % (pdb)
...
def FindConservedWaters(selectedStruturePDB,selectedStrutureChain,seq_id,resolution,refinement,user_def_list,clustering_method,inconsistency_coefficient,prob,save_sup_files=True):# e.g: selectedStruturePDB='3qkl',selectedStrutureChain='A'
    ...
    print "online_pdb_db",online_pdb_db
    ## Display imput parameters
...
class ConservedWaters( Frame ):
    ...
    def makeWindow(self):
        ...
        #print "v1",str(v1.get()).lower()
        for i in range(10):
            newfield='v'+str(i+1)
            #print newfield, eval(newfield).get()
            setattr( self, newfield, eval(newfield) )
        #default for debug
        v1.set('4lyw')
        v2.set('a')
        v4.set('1.1')
        #########################
        Button(frame2,text=" Find Conserved Water Molecules ", command = self.launch_fcw_czq,).grid(row=1, column=1, sticky=W)
        
        
#        Button(frame2,text=" Find Conserved Water Molecules ",
#                command = lambda: thread.start_new_thread(FindConservedWaters,(
#                    str(v1.get()).lower(),
#                    str(v2.get()).upper(),
#                    str(v3.get()),
#                    float(v4.get()),
...
#            ).grid(row=1, column=1, sticky=W)
    def launch_fcw_czq(self):
        print self.v1.get().lower()
        print self.v2.get().upper()
        
#        FindConservedWaters(str(self.v1.get().lower()),str(self.v2.get().upper()),
#                            str(self.v3.get()), float(self.v4.get()),
#                            str(self.v5.get()),str(self.v6.get()),str(self.v7.get()),
#                            float(self.v8.get()),float(self.v9.get()),
#                            bool(self.v10.get())
#                            )
        
        thread.start_new_thread(FindConservedWaters,
                               (str(self.v1.get().lower()),str(self.v2.get().upper()),
                            str(self.v3.get()), float(self.v4.get()),
                            str(self.v5.get()),str(self.v6.get()),str(self.v7.get()),
                            float(self.v8.get()),float(self.v9.get()),
                            bool(self.v10.get())
...
def toPyWATER( v1, v2, v3 = '95', v4 = 2.0, v5 = 'Mobility', v6 = '', v7 = 'complete', v8 = 2.0, v9 = 0.7):
    ...
    seq_id = str(v3)   #sequence identity
    resolution = float(v4)
    refinement = str(v5)
    ...
    prob = float(v9)
    #should use thread to avoid not responding
    thread.start_new_thread(FindConservedWaters, (selectedStruturePDB,selectedStrutureChain,seq_id,resolution,refinement,user_def_list,clustering_method,inconsistency_coefficient,prob))
